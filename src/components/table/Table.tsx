...
export type ExtendColumn = Column & {
    filterOptions?: {
        label?: string;
        type?: FieldType;
        isMultiple?: boolean;
...
const Table = <TableValues extends object = {}>(props: TableProps<TableValues>) => {
    ...
    /*
    * Call fetch data when page index or page size change
    */
    useEffect(() => {
        if (fetchData) {
            fetchData({
                page: dataTable.state.pageIndex,
                perpage: dataTable.state.pageSize,
                ...filters
            })
        }
    }, [dataTable.state.pageIndex, dataTable.state.pageSize, filters])
    
    /*
    * Re calc pageCount when prop 'totalRecords' or page size change
    */
    useEffect(() => {
        setPageCount(Math.ceil((props['totalRecords'] || 0) / (dataTable.state.pageSize)))
    }, [props['totalRecords'], dataTable.state.pageSize])
    ...
    return (
        <>
            <div className="d-flex mb-2">
                ...
                {isFilterable && <FilterModal schemas={props['columns']} onFilter={onFilter}/>}
            </div>
