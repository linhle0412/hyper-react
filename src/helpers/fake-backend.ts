...
export function configureFakeBackend() {
    ...
    mock.onGet(RegExp(`/products\?.*`)).reply(function (config) {
        return new Promise(function (resolve, reject) {
            setTimeout(function () {
                // get parameters from get request
                const queries = parseQueryString(config.url || '')
                console.log(queries);
                
                const page = parseInt(queries.page)
                const perpage = parseInt(queries.perpage)
                let filterProducts = productsMock.slice(page * perpage, page * perpage + perpage)
                
                resolve([200, {
                    total: productsMock.length,
    ...
    mock.onPost('/products/delete').reply(function (config) {
        return new Promise(function (resolve, reject) {
            setTimeout(function () {
                productsMock = productsMock.filter((product) => !JSON.parse(config.data).ids.includes(product.id))
